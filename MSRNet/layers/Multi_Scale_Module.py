...
from layers.Multi_Scale_layers import *
...
class Multi_Scale_Module(nn.Module):
    def __init__(self, c_in: int, context_window: int, target_window: int,
                 period, scale_weight, patch_len, stride, kernel_list, serial_conv=False, wo_conv=False, add=False,
        ...
        self.stride_list = [period // 2 for period in self.period_list]

        self.tokens_list = [int((context_window - period) / stride + 2)
                            for period, stride in zip(self.period_list, self.stride_list) if stride != 0]

        self.backbone = nn.ModuleList([nn.Sequential(
            TSTiEncoder(c_in, patch_num=token, patch_len=self.period_list[i], max_seq_len=max_seq_len,
        ...
        self.head = Head(context_window, len(period), target_window, c_in, scale_weight=scale_weight,
                         head_dropout=head_dropout, Concat=not add)
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        if self.wo_conv:
            ...
            for i, (period, stride, tokens) in enumerate(zip(self.period_list, self.stride_list, self.tokens_list)):

                seq_len = z.shape[-1]

                # Calculate padding
                # num_patches = (seq_len - period) // stride + 1
                # num_patches = int((seq_len - period) / stride + 2)
                total_length = (tokens - 1) * stride + period
                pad_len = total_length - seq_len

                # Pad sequence
                if pad_len > 0:
                    pad = z[:, :, -1:].repeat(1, 1, pad_len)
                    z_padded = torch.cat([z, pad], dim=-1)
                else:
                    z_padded = z

                # Create patches
                patches = z_padded.unfold(dimension=-1, size=period, step=stride)
                patches = patches.permute(0, 1, 3, 2)

                # print("patches.shape", patches.shape) # patches.shape torch.Size([96, 1, 8, 24])
                glo = self.backbone[i](patches)
                # print("glo.shape", glo.shape) # glo.shape torch.Size([32, 1, 96])
                res.append(glo)

        # denorm
        # print("res.shape", res[0].shape)
        # print("res.shape", res[0].shape) # res.shape torch.Size([32, 1, 96])
        z = self.head(res)
        z = z.permute(0, 2, 1)
...
class Head(nn.Module):
    def __init__(self, context_window, num_period, target_window, nvars, scale_weight=None, head_dropout=0, Concat=True):
        super().__init__()
        self.Concat = Concat
        self.nvars = nvars
        self.linears = nn.ModuleList([
            nn.Linear(context_window * (num_period if Concat else 1), target_window) for _ in range(nvars)
        ])
    ...
    def forward(self, x):  # x: [batch_size, nvars, d_model, patch_num]
        if self.Concat:
            x = torch.cat(x, dim=-1)  # Assuming x is a list of tensors
            # print("x.shape", x.shape) # x.shape torch.Size([32, 1, 288])
        else:
        ...
        for i in range(self.nvars):
            xi = x[:, i, :]  # Shape: [batch_size, input_dim]
            xi = self.linears[i](xi)  # Shape: [batch_size, target_window]
            outputs.append(xi.unsqueeze(1))  # Shape: [batch_size, 1, target_window]

        x = torch.cat(outputs, dim=1)  # Shape: [batch_size, nvars, target_window]
        x = self.dropout(x)
        return x
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        # print(f"W_P weight.shape: {self.W_P.weight.shape}")
        ...
        # print("z.shape", z.shape) # z.shape torch.Size([32, 1, 32, 8])
        return z
