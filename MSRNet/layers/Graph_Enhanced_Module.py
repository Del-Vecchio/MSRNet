...
class GraphBlock(nn.Module):
    def __init__(self, c_out, d_model, conv_channel, skip_channel,
        ...
        self.node_emb = nn.Parameter(torch.randn(c_out, node_dim), requires_grad=True)
        # Initialize graph constructor
        self.graph_constructor = graph_constructor(nnodes=c_out, k=k, dim=node_dim, alpha=alpha)
    ...
    def forward(self, x):
        # Generate adjacency matrix using graph_constructor
        adp = self.graph_constructor(self.node_emb)
